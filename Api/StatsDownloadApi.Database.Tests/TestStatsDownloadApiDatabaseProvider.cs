namespace StatsDownloadApi.Database.Tests
{
    ...
    public class TestStatsDownloadApiDatabaseProvider
    {
        ...
        public void GetFoldingMembers_WhenInvoked_GetsFoldingUsers()
        {
            databaseConnectionServiceMock.When(service =>
                ...
                callInfo =>
                {
                    ...
                    dataTable.Columns.Add(new DataColumn("FriendlyName",
                        typeof (string)));
                    dataTable.Columns.Add(new DataColumn("BitcoinAddress",
                        typeof (string)));
                    dataTable.Columns.Add(new DataColumn("PointsGained", typeof (long)));
                    dataTable.Columns.Add(
                        new DataColumn("WorkUnitsGained", typeof (long)));
        ...
        {
            databaseConnectionServiceMock.When(service => service.ExecuteStoredProcedure("[FoldingCoin].[GetMembers]",
                Arg.Any<IEnumerable<DbParameter>>(), Arg.Any<DataTable>())).Do(callInfo =>
            {
                var dataTable = callInfo.Arg<DataTable>();

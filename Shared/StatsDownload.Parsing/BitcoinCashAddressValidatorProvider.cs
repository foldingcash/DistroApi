namespace StatsDownload.Parsing
{
    ...
    public class BitcoinCashAddressValidatorProvider : IBitcoinCashAddressValidatorService
    {
        private readonly ILogger logger;

        public BitcoinCashAddressValidatorProvider(ILogger<BitcoinCashAddressValidatorProvider> logger)
        {
            this.logger = logger;
        }
        ...
        public bool IsValidBitcoinCashAddress(string address)
        {
            ...
            {
                logger.LogWarning(ex, "There was an exception while validating the Bitcoin Cash address. {address}");
                return false;
            }
        }
        ...
        private byte[] ConvertBitsFiveToEight(byte[] bytes)
        {
            ...
            if (bytes[a2 + 1] % 4 != 0)
            {
                throw new Exception("Invalid CashAddr.");
            }
        ...
        private string OldAddressToCashAddress(string oldAddress, out bool isP2PKH, out bool mainnet)
        {
            ...
            var ret = new StringBuilder(mainnet ? "bitcoincash:" : "bchtest:");

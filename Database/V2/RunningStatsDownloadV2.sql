...
--EXEC [FoldingCash].[FileDownloadError] @DownloadId, 'File Download Error Message';

-- When the file download is finished, upload the file data
PRINT 'File download finished'
EXEC [FoldingCash].[FileDownloadFinished] @DownloadId
	,'\\storage\location\unprocessed\FileName.ext'
	,'FileName'
	,'.ext';

PRINT 'File validation started'
EXEC [FoldingCash].[FileValidationStarted] @DownloadId
...
EXEC [FoldingCash].[FileValidated] @DownloadId
	,@ValidatedDateTime
...
--EXEC [FoldingCash].[FileValidationError] @DownloadId, 'File Validation Error';
...
EXEC [FoldingCash].[GetValidatedFiles] @StartDate, @EndDate;